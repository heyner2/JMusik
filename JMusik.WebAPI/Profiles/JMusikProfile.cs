...
{
    //hereda de Profile que pertenece a Mapper 
    public class JMusikProfile: Profile
    {
        ...
        public JMusikProfile()
        {
            ...
            this.CreateMap<Orden, OrdenDto>().ForMember(u => u.Usuario, p => p.MapFrom(m => m.Usuario.Nombre)).
                ReverseMap().ForMember(u => u.Usuario, p => p.Ignore());
            this.CreateMap<DetalleOrden, DetalleOrdenDto>().ForMember(u => u.Producto, p => p.MapFrom(m => m.Producto.Nombre)).
                ReverseMap().ForMember(u => u.Producto, p => p.Ignore());
